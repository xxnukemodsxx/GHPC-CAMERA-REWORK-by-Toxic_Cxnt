...
[assembly: MelonInfo(typeof(GHPCLiveAARV2.Mod), "GHPC Live AAR", "2.0.0", "Toxic_Cxnt")]
...
namespace GHPCLiveAARV2
{
    public sealed class Mod : MelonMod
    {
        ...
        {
            LiveAar.Initialize();
            MelonLogger.Msg("GHPC Live AAR v2.0.0 loaded.");
        }
    ...
    internal static class LiveAar
    {
        ...
        private static AarStage _stage;
        ...
        internal static void Tick()
        {
            ...
            for (int i = _pending.Count - 1; i >= 0; i--)
            {
                ...
                object target = FindTargetVehicle(p.Shot);
                if (target == null && p.Attempts < 6)
                {
                    p.Attempts++;
                    p.ReadyUtc = now.AddMilliseconds(120);
                    continue;
                }
        ...
        public static void ShotEnded(object __instance)
        {
            try
            {
                ...
                // Give GHPC a short moment to register child/spall/jet shots before snapshotting.
                _pending.Add(new PendingShot(__instance, DateTime.UtcNow.AddMilliseconds(340)));
                while (_pending.Count > 8)
                {
        ...
        private static object FindTargetVehicle(object shot)
        {
            List<object> frames = R.List(R.Get(shot, "AllShotFrames"));
            for (int i = frames.Count - 1; i >= 0; i--)
            {
                object v = R.Get(frames[i], "HitVehicle");
                if (v != null) return v;
            }
        ...
        {
            try { MelonLogger.Msg("[LiveAAR] " + s); } catch { }
        }
    }
    ...
    internal static class ShotHook
    {
        ...
        internal static bool TryInstall()
        {
            ...
            if (_harmony == null)
            {
                ...
                _harmony = hc.Invoke(new object[] { "ghpc.liveaar.v2" });
            }
            ...
            for (int i = 0; i < pms.Length; i++)
            {
                ...
                if (ps.Length >= 3 && typeof(MethodBase).IsAssignableFrom(ps[0].ParameterType) && ps[1].ParameterType == hmType && ps[2].ParameterType == hmType)
                { patch = pms[i]; break; }
            }
