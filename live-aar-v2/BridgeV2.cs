...
namespace GHPCLiveAARV2
{
    internal static class R
    {
        internal static Type Find(string name)
        {
            ...
            for (int i = 0; i < ass.Length; i++)
            {
                try
                {
                    t = ass[i].GetType(name, false);
                    if (t != null) return t;
                    Type[] ts = ass[i].GetTypes();
        ...
        internal static bool Set(object o, string n, object v)
        {
            ...
            {
                PropertyInfo p = t.GetProperty(n, f);
                if (p != null && p.CanWrite) { p.SetValue(o, Coerce(v, p.PropertyType), null); return true; }
            }
            catch { }
            try
            {
                FieldInfo x = t.GetField(n, f);
                if (x != null) { x.SetValue(o, Coerce(v, x.FieldType)); return true; }
            }
            catch { }
            return false;
        }
        ...
        {
            if (o == null) return null;
            return Invoke(o.GetType(), o, n, args, false);
        }

        internal static object CallStatic(Type t, string n, params object[] args)
        {
            if (t == null) return null;
            return Invoke(t, null, n, args, true);
        }
        ...
        private static object Coerce(object v, Type t)
        {
            ...
            if (t.IsPrimitive || t == typeof(decimal)) return Convert.ChangeType(v, t, CultureInfo.InvariantCulture);
            return v;
        }
    ...
    internal static class U
    {
        ...
        private static Type _go, _obj, _vec3, _quat, _rect, _material, _shader, _line, _camera, _light, _lightType;
        private static Type _renderer, _meshRenderer, _meshFilter, _skin, _mesh, _skybox;
        ...
        internal static void Init()
        {
            ...
            _mesh = R.Find("UnityEngine.Mesh");
            _skybox = R.Find("UnityEngine.Skybox");
        ...
        internal static object AddComponent(object go, Type t) { return go == null || t == null ? null : R.Call(go, "AddComponent", t); }
        ...
        {
            object v = R.GetStatic(R.Find("UnityEngine.Screen"), "width");
            try { return Convert.ToSingle(v, CultureInfo.InvariantCulture); } catch { return 1920f; }
        }

        internal static float ScreenH()
        {
            object v = R.GetStatic(R.Find("UnityEngine.Screen"), "height");
            try { return Convert.ToSingle(v, CultureInfo.InvariantCulture); } catch { return 1080f; }
        }

        internal static object Shader(string name) { Init(); return R.CallStatic(_shader, "Find", name); }
        ...
        internal static void Color(object m, float r, float g, float b, float a)
        {
            ...
            if (co == null) return;
            R.Set(m, "color", co);
            R.Call(m, "SetColor", "_Color", co);
            R.Call(m, "SetColor", "_BaseColor", co);
        }

        internal static object SolidMaterial(float r, float g, float b, float a)
        {
            object sh = Shader("Unlit/Color");
            if (sh == null) sh = Shader("Sprites/Default");
            object m = NewMaterial(sh);
            if (m != null) Color(m, r, g, b, a);
            return m;
        }

        internal static Array MaterialArray(object m, int count)
        {
            Init();
            Array a = Array.CreateInstance(_material, Math.Max(1, count));
        ...
        internal static object NewMesh()
        {
            Init();
            try { return _mesh == null ? null : Activator.CreateInstance(_mesh); }
            catch { return null; }
        }

        internal static object BakeSkin(object skin)
        {
            object mesh = NewMesh();
            if (mesh == null || skin == null) return null;
            try
            {
                R.Call(skin, "BakeMesh", mesh);
                return mesh;
            }
            catch { Destroy(mesh); return null; }
        }
        ...
        internal static object Camera(string name)
        {
            Init();
            object go = GO(name);
            return AddComponent(go, _camera);
        }

        internal static void SolidBlack(object cam)
        {
            if (cam == null) return;
            object co = ColorObject(.008f, .010f, .012f, 1f);
            if (co != null) R.Set(cam, "backgroundColor", co);
            R.Set(cam, "clearFlags", 2);
            object go = R.Get(cam, "gameObject");
            object sky = GetComponent(go, _skybox);
            if (sky != null) R.Set(sky, "enabled", false);
        }

        internal static void DepthOnly(object cam)
        {
            if (cam == null) return;
            R.Set(cam, "clearFlags", 3);
            object go = R.Get(cam, "gameObject");
            object sky = GetComponent(go, _skybox);
            if (sky != null) R.Set(sky, "enabled", false);
        }
        ...
        internal static object Line(string name, List<object> points, float width, float r, float g, float b, float a, int layer, object material)
        {
            Init();
            ...
            if (material != null) R.Set(lr, "material", material);
            object co = ColorObject(r, g, b, a);
            if (co != null)
            {
